        ...
        while (true) {
            ...
            if (operStack.isOper(ch)) { //如果是运算符
                ...
                if (!operStack.isEmpty()) {
                    ...
                    //就需要从数栈中pop出两个数，在从符号栈中pop出一个符号，进行运算，将得到结果入数栈
                    //然后将当前的操作符入符号栈
                ...
                //如果是数，则直接入数栈
                //numStack.push(ch - 48); //? '1' = 49.tochar
                //分析思路：
                //1.当处理多位数时，不能发现是一个数就立即入栈，因为他可能是多位数
                //2.在处理数，需要向expression的表达式的index后再看一位，如果是数酒精性扫描，如果是符号才入栈
                //3.因此我们需要定义一个变量字符串，用于拼凑
...
class ArrayStack2 {
    ...
    //构造器 
    public ArrayStack2(int pmaxSize) {
